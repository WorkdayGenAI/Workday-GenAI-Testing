...
## New User Guide: Create This Agent in GenWizard

This section is for a developer or platform administrator who has received the Python tool
files in this repository and wants to create the same GTS agent in a GenWizard environment.
The exact button names can vary by GenWizard release and by organisation, but the sequence is
the same: create an agent, register the tool files, configure secrets, publish, and test each
supported flow.

### 1. Understand the solution before registering it

GTS generates Workday test cases as a CSV file. The repository contains several agent variants;
choose the one that matches your source data and target domain:

| Use case | Folder to register | Tools to register |
|---|---|---|
| HR business process library or live Workday definition | `GTS Business Process` | `wdquasar_reposcenarios`, `wd_bp_quasar_to_workday_xml`, `wd_bp_quasar_xml_to_csv_pll`, `workday_send_msg_to_canvas_wd` |
| Finance business process library or live Workday definition | `GTS_TestCaseGeneration_BusinessProcess_Finance` | `wdquasar_reposcenarios_fin`, `wd_bp_quasar_to_workday_xml_fin`, `wd_bp_quasar_xml_to_csv_pll_fin`, `workday_send_msg_to_canvas_fin` |
| User story or HR business process library | `SA-GTS_UserStory_Businessprocess` | `wd_bp_quasar_to_workday_csv_us`, `wdquasarbp_to_testcases_csv`, `workday_send_msg_to_canvas` |
| Uploaded Workday step spreadsheet | `Workday Test Scenario Generator` | `wd_test_scenerio_generation_to_csv`, `workday_send_msg_to_canvas` |
| Uploaded spreadsheet with Claude Code and Bedrock | `SA - Workday TestScenario CWB` | `gp_file_download_CM_WD` |

Use one folder as one agent unless your platform team has a reason to combine variants. The
tools in a folder share a purpose and configuration, while the Finance and CWB variants have
different dependencies and credentials.

### 2. Gather the platform prerequisites

Before opening GenWizard, confirm the following with the platform and Workday administrators:

- Access to create, configure, and publish agents in GenWizard.
- Access to the GenWizard secret vault and artifact storage.
- The platform Python runtime and the `core` SDK used by the tool scripts.
- Network access from the runtime to Quasar, Workday, the Claude endpoint, and Canvas.
- A test Canvas conversation or equivalent test channel.
- The correct knowledge-base index and source file for the selected variant.
- For CWB only: a Linux runtime with Node.js/npm, `pyte`, and permission to install and run
      Claude Code through Amazon Bedrock. Do not deploy this variant on Windows or macOS.

Do not put passwords, API keys, or endpoint credentials in the Python files or in the agent
prompt. The scripts retrieve them with `core.get_secret(...)` at runtime.

### 3. Create the agent in GenWizard

1. In GenWizard, create a new agent or assistant and give it a clear name, such as
       `GTS - Workday Test Case Generator - HR`.
2. Add a short description: *Generate Workday test cases from a business process name, user
       story, or uploaded process spreadsheet and return a downloadable CSV.*
3. Select the Python-capable tool/runtime option used by your organisation. The scripts import
       `core`, so they must run inside the GenWizard tool host rather than as ordinary local Python
       programs.
4. Configure artifact storage for uploaded files and generated files. The upload tools expect
       uploaded files under `uploaded_artifacts/`; generated files are written under `artifacts/`.
5. Add the tool files from the selected folder. These files intentionally have no `.py` suffix
       in this repository; upload them as Python tool scripts if GenWizard asks for a language.
6. For each tool, expose the function parameters shown in the tool's source or the tool table
       below. Keep parameter names unchanged because they are part of the tool contract.
7. Save the agent as a draft until secrets and the smoke tests are complete.

### 4. Register the tools and their contracts

Use the following contracts when GenWizard asks for tool metadata. `conversationid` is the
conversation identifier supplied by the platform and must be passed unchanged between stages.

| Tool | Required inputs | Purpose |
|---|---|---|
| `wdquasar_reposcenarios` / `_fin` | `payload`, `BusinessProcessName`, `conversationid`, `fileinitialname` | Search the scenario library and generate a CSV |
| `wd_bp_quasar_to_workday_xml` / `_fin` | `bp_input_json`, `filename`, `conversationid` | Fetch a Workday BP definition and save extracted steps |
| `wd_bp_quasar_xml_to_csv_pll` / `_fin` | `conversationid` | Read stage-1 steps and generate the final CSV |
| `wd_bp_quasar_to_workday_csv_us` | `fileinitialname`, `conversationid`, `BusinessProcessName`, `payload` | Generate fresh cases from a user story |
| `wdquasarbp_to_testcases_csv` | `payload`, `BusinessProcessName`, `conversationid`, `fileinitialname` | Search the HR library and generate cases |
| `wd_test_scenerio_generation_to_csv` | `filename`, `conversationid` | Read an uploaded spreadsheet and generate cases |
| `gp_file_download_CM_WD` | `filename`, `conversation_id` | Run the CWB/Bedrock upload flow; Linux only |
| `workday_send_msg_to_canvas*` | `conversation_id`, `message`, optional `environment`, `recipient_id` | Post the result or download link to Canvas |

When GenWizard supports tool descriptions, describe the sender tool as a delivery tool rather
than a second generator. The generator should return its artifact URL or result message to the
agent, and the agent should call the sender with the same conversation context.

### 5. Add secrets to the GenWizard vault

Create these entries using the exact names expected by the scripts. Obtain actual values from
your platform administrators; the repository intentionally does not contain them.

| Secret | Used for | Required contents |
|---|---|---|
| `wd-tool-secrets` | Quasar | Base URL, username, password or token as required by the Quasar deployment |
| `SCA_WA_Claude_Opus_4_6` | Claude for Helpers 1-4 | Endpoint, API key, deployment/model name |
| `wd_dnt_10` | Workday BP definition path | Workday URL, username, password |
| `Bedrock Sonnet 4.5` | CWB only | AWS region, access key, secret key, model details |
| `workday`, `workday_CUI`, or environment-specific Canvas secret | Canvas delivery | Canvas URL, username, password or token |

Canvas secret selection is environment-specific. The sender scripts contain the supported names
for `prod`, `demo`, `stagetest`, `dev`, `sapcui`, `platforms-dev`, and `next-gen`. Verify the
selected folder's sender script before adding only the secrets needed by your deployment.

After saving each secret, run the platform's secret-access test if available. A secret name that
exists but contains the wrong field names is effectively missing to the script.

### 6. Configure the knowledge source and environment

Configure the data sources required by the selected agent before publishing:

- HR library: `HCM_Test_Scenario_Library_Consolidated.xlsx` and index
      `wd_TSEndtoEndTesting_idx`.
- Finance library: `Vectorization_Repository___30th_Oct_2025.xlsx` and index
      `wd_TSEndtoEndTesting_Finance_idx`.
- Workday definition path: the Quasar BP definition lookup and the Workday custom report
      configured in the script (`JN_Business_Process_Definition_Detail`).
- Upload paths: artifact upload enabled, with the input file available under
      `uploaded_artifacts/<filename>`.

If your GenWizard tenant uses different index names, report names, or artifact prefixes, update
the corresponding tool configuration and test that variant separately. Do not silently point an
HR agent at the Finance index.

### 7. Add agent instructions and routing guidance

Give the agent a short routing instruction so it selects the correct tool:

```text
You generate Workday test cases. Use the HR or Finance library tool when the user provides a
business process name and the matching library is configured. Use the user-story tool when the
user provides a user story. Use the spreadsheet tool when the user uploads an XLSX process
definition. For a live Workday definition, run stage 1 before stage 2 and preserve the same
conversationid. Return the generated CSV download link. Ask for missing input instead of
inventing a business process name or file.
```

Add these operational rules to the agent configuration:

- Preserve `conversationid` exactly across a two-stage Workday-definition request.
- Do not expose secret values in chat responses or logs.
- Tell users that generated test cases are drafts and require Workday/business review.
- For uploads, confirm the filename and that the file is an XLSX containing process steps.
- For a failed library lookup, suggest the user-story or Workday-definition path rather than
      fabricating scenarios.

### 8. Publish and run smoke tests

Publish the draft to a non-production environment first. Run one small test for each flow that
you enabled:

1. **HR library:** request test cases for one known process such as `Hire Employee`.
2. **Finance library:** request one known Finance business process.
3. **Workday definition:** run stage 1, then stage 2 using the same conversation.
4. **User story:** provide a short story with an actor, action, and acceptance criteria.
5. **Upload:** upload a small valid XLSX and verify the generated CSV link.
6. **Canvas delivery:** verify that the result appears in the same conversation.

For each test, verify the tool run status, artifact creation, CSV contents, download link, and
server logs. A successful HTTP response alone is not enough: the CSV must contain complete rows
and the link must be accessible to the intended user.

### 9. Promote safely

After the smoke tests pass, record the agent version, selected folder, tool names, index/report
names, and secret names in your deployment record. Promote the same tested configuration through
test and production environments using the platform's normal approval process. Keep the source
scripts and GenWizard agent version aligned so a later tool update does not leave the registered
agent running an older file.

### 10. Common setup failures

| Symptom | Likely cause | Check |
|---|---|---|
| Tool cannot import `core` | Script was run outside the GenWizard host | Register it as a GenWizard Python tool |
| Authentication error | Missing secret or wrong field names | Compare vault entry names and fields with the selected script |
| No scenarios found | Wrong process name, index, or library | Test with a known process and verify the configured index |
| Stage 2 cannot find input | Stage 1 was skipped or conversation IDs differ | Re-run both stages with the same `conversationid` |
| Upload tool cannot find file | Artifact path or filename mismatch | Confirm `uploaded_artifacts/<filename>` and the exact filename passed to the tool |
| Download link is missing | Artifact write or Canvas sender failed | Inspect artifact creation first, then sender logs |
| CWB fails with `pty`, `fcntl`, or `termios` | Non-Linux runtime or missing system dependency | Deploy CWB on Linux and verify Node.js/npm and `pyte` |

Do not place real credentials in issue tickets, screenshots, README files, or generated prompts.
Rotate any credential that appears in a log and remove the exposed value from the log store.

## Contents (click to jump)

1. [New User Guide: Create This Agent in GenWizard](#new-user-guide-create-this-agent-in-genwizard)
2. [What Does This Do?](#section-1--what-does-this-do)
3. [The Story of How It Works](#section-2--the-story-of-how-it-works)
4. [What You Get Out of It](#section-3--what-you-get-out-of-it)
5. [What Can It Do? (The Five Helpers)](#section-4--what-can-it-do-the-five-helpers)
6. [The Big Picture](#section-5--the-big-picture)
7. [Before You Start (Your Checklist)](#section-6--before-you-start-your-checklist)
8. [How to Set It Up](#section-7--how-to-set-it-up)
9. [Settings You Need to Fill In](#section-8--settings-you-need-to-fill-in)
10. [What Could Go Wrong](#section-9--what-could-go-wrong)
11. [Why This Matters](#section-10--why-this-matters)
12. [Known Limitations and Watch-Outs](#section-11--known-limitations-and-watch-outs)
13. [Word List (Plain-English Dictionary)](#word-list-plain-english-dictionary)
